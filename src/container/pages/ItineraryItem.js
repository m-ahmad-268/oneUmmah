...
import { generateCodeItem, getAllItem, getAllItemType, getAllMenuType, getAllRoles, saveItemType, saveItinerayItem, updateItemType, updateItinerayItem } from '../../services/commonService';
...
function ItineraryItem() {
        ...
        if (isModalVisible) {
            if (editingFood) {
                form.setFieldsValue({
                    ...
                    serItineraryItemTypeId: editingFood.serItineraryItemTypeId || '',
                ...
                (async () => {
                    try {
                        let code;
                        const data = await generateCodeItem();
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            const data = isEditMode ? await updateItinerayItem({
                serItineraryItemId: editingFood?.serItineraryItemId,
                ...req,
            }) : await saveItinerayItem(req);
        ...
        <>
            <CardToolbox>
                <PageHeader
                    ...
                    subTitle={
                        <>
                            <span className="title-counter">{arrItems.length} Itinerary Item</span>
                        </>
                    }
                    buttons={[
                        <>
                ...
                >
                    <Row gutter={16}>
                        ...
                        <Col xs={24} sm={12}>
                            ...
                            >
                                <Input placeholder="Enter Itinerary name" disabled={isViewMode} />
                            </Form.Item>
                        </Col>
                        <Col xs={24} sm={12}>
                            <Form.Item
                                name="serItineraryItemTypeId"
                                label="Itinerary Type"
                                rules={[{ required: true, message: 'Please select Itinerary Type' }]}
                            >
                                <Select
                                    // value={activeType}
                                    // onChange={(value) => setActiveType(value)}
