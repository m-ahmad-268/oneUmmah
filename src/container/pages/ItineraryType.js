...
import { generateCodeItemType, getAllItemRoles, getAllItemType, getAllMenuType, getAllRoles, saveItemType, updateItemType } from '../../services/commonService';
...
function ItineraryType() {
    ...
    useEffect(() => {
        if (isModalVisible) {
            if (editingFood) {
                ...
                (async () => {
                    let code;
                    const data = await generateCodeItemType();
    ...
    const handleSubmit = async (values) => {
        ...
        try {
            ...
            const data = isEditMode ? await updateItemType({
                serItineraryItemTypeId: editingFood?.serItineraryItemTypeId,
                ...req,
            }) : await saveItemType(req);
        ...
        <>
            <CardToolbox>
                <PageHeader
                    ...
                    subTitle={
                        <>
                            <span className="title-counter">{arrItems.length} Itinerary Types</span>
                        </>
                    }
                    buttons={[
                        <>
                ...
                >
                    <Row gutter={16}>
                        <Col xs={24} sm={12}>
                            ...
                            >
                                <Input placeholder="Enter role code" disabled={true} />
                            </Form.Item>
                        </Col>
                        <Col xs={24} sm={12}>
                            <Form.Item
