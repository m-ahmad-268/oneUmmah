// src/container/pages/DecorServiceFormModal.js
...
function DecorServiceFormModal({ visible, initialData, onCancel, onOk }) {
  ...
  useEffect(() => {
    if (visible) {
      if (initialData) {
        ...
        form.resetFields();
        form.setFieldsValue({ blnIsActive: false });
      }
    }
  ...
  const handleSave = async (values) => {
    ...
    try {
      ...
      if (data.code === 200) {
        message.success(initialData ? 'Service updated successfully!' : 'Service added successfully!');
        form.resetFields();
        onOk();
      } else {
...
export default DecorServiceFormModal;
