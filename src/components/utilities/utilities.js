...
const customTooltips = function (context) {
  ...
  // --- FIX START ---
  // Access the chart instance from the 'context' object
  const chart = context.chart;

  // Add a crucial check for chart and canvas before proceeding
  if (!chart || !chart.canvas) {
    return; // Exit if chart or canvas is not available
  }

  const container = chart.canvas.closest('.ninjadash-chart-container');
  // --- FIX END ---
  ...
  if (tooltipEl !== null) {
    ...
    if (tooltipModel.body) {
      ...
      // Add a check for tableRoot as well, just in case
      if (tableRoot) {
    ...
    // --- FIX START ---
    // Use the 'chart' instance derived from 'context.chart'
    ...
    const toolTipHeight = toolTip ? toolTip.clientHeight : 0; // Add check for toolTip
